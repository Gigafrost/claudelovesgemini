...
class FieldMapper {
  constructor() {
    ...
    this.fieldMappings = {
      ...
      website: {
        keywords: ['website', 'personal website', 'homepage', 'web site', 'personal site', 'site url'],
        weight: 0.9
      },
      portfolio: {
        keywords: ['portfolio', 'portfolio url', 'portfolio website', 'portfolio site', 'behance', 'dribbble'],
        weight: 0.95
      },
      resume: {
  ...
  /**
   * Classify a real DOM element.
   * Uses strong signals first (autocomplete / input type) and then fuzzy keywords.
   *
   * Returns: { type, confidence, reasons, signals }
   */
  classifyElement(element, labelText = '') {
    const reasons = [];
    if (!element) {
      return { type: 'other', confidence: 0, reasons: ['no-element'], signals: {} };
    }

    const tag = (element.tagName || '').toLowerCase();
    const inputType = (element.type || '').toLowerCase();
    const name = (element.getAttribute?.('name') || element.name || '').toString();
    const id = (element.getAttribute?.('id') || element.id || '').toString();
    const placeholder = (element.getAttribute?.('placeholder') || element.placeholder || '').toString();
    const autocomplete = (element.getAttribute?.('autocomplete') || '').toLowerCase();

    const signals = { tag, inputType, name, id, placeholder, autocomplete, label: labelText };

    // 1) Autocomplete signals (highest confidence)
    const acMap = {
    ...
    if (autocomplete && acMap[autocomplete]) {
      reasons.push(`autocomplete:${autocomplete}`);
      return { type: acMap[autocomplete], confidence: 0.99, reasons, signals };
    }

    // 2) Input type signals
    if (inputType === 'email') {
      reasons.push('inputType:email');
      return { type: 'email', confidence: 0.98, reasons, signals };
    }
    if (inputType === 'tel') {
      reasons.push('inputType:tel');
      return { type: 'phone', confidence: 0.98, reasons, signals };
    }
    if (inputType === 'file') {
      const txt = `${labelText} ${name} ${id}`.toLowerCase();
      if (txt.includes('resume') || txt.includes('cv')) {
        reasons.push('inputType:file + resume');
        return { type: 'resume', confidence: 0.97, reasons, signals };
      }
      if (txt.includes('cover')) {
        reasons.push('inputType:file + cover');
        return { type: 'coverLetter', confidence: 0.9, reasons, signals };
      }
      reasons.push('inputType:file');
      return { type: 'resume', confidence: 0.6, reasons, signals };
    }
    if (inputType === 'url') {
      // Use text hints to differentiate LinkedIn/portfolio/website
      const txt = `${labelText} ${name} ${id} ${placeholder}`.toLowerCase();
      if (txt.includes('linkedin')) {
        reasons.push('inputType:url + linkedin');
        return { type: 'linkedIn', confidence: 0.95, reasons, signals };
      }
      if (txt.includes('github')) {
        reasons.push('inputType:url + github');
        return { type: 'github', confidence: 0.9, reasons, signals };
      }
      if (txt.includes('portfolio') || txt.includes('behance') || txt.includes('dribbble')) {
        reasons.push('inputType:url + portfolio');
        return { type: 'portfolio', confidence: 0.9, reasons, signals };
      }
      reasons.push('inputType:url');
      // fall through to fuzzy
    }

    // 3) Fuzzy keyword matching on combined text
    const fuzzy = this.classifyField(labelText, name, id, placeholder);
    if (fuzzy && fuzzy.confidence > 0) {
      return { ...fuzzy, reasons: ['fuzzy'], signals };
    }

    return { type: 'other', confidence: 0.0, reasons: ['no-match'], signals };
  }

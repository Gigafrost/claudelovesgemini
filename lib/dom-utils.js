...
class DomUtils {
   ...
   * @param {Element} control - The form control element
   * @param {Document|ShadowRoot} rootContext - Optional root context for searching (defaults to document)
   */
  static inferLabel(control, rootContext = null) {
    const sources = [];
    if (!control) return { label: '', sources };

    // Determine the root to search in - use shadow root if control is in one
    const root = rootContext || this.getRootContext(control) || document;
    ...
    // 2) <label for="id"> - search in the appropriate root context
    if (control.id) {
      let forLabel = null;
      try {
        forLabel = root.querySelector(`label[for="${CSS.escape(control.id)}"]`);
        // Also try document if root is different
        if (!forLabel && root !== document) {
          forLabel = document.querySelector(`label[for="${CSS.escape(control.id)}"]`);
        }
      } catch (e) {}
    ...
    // 4) aria-labelledby - search in appropriate context
    ...
    if (ariaLabelledBy) {
      ...
      ids.forEach(id => {
        let el = null;
        try {
          el = root.getElementById ? root.getElementById(id) : root.querySelector(`#${CSS.escape(id)}`);
          // Also try document if root is different
          if (!el && root !== document) {
    ...
    const heuristic = this.inferLabelFromNearbyText(control, root);
  ...
  /**
   * Get the root context (Document or ShadowRoot) for an element
   * Handles elements in Shadow DOM, iframes, and the main document
   */
  static getRootContext(element) {
    if (!element) return document;

    // Use native getRootNode() if available - this handles shadow DOM and iframes correctly
    if (typeof element.getRootNode === 'function') {
      const root = element.getRootNode();
      // getRootNode returns either a Document, ShadowRoot, or the element itself (if detached)
    ...
    while (node) {
      ...
      // Check if we've reached the ownerDocument
    ...
    // Final fallback - try element's ownerDocument
    return element.ownerDocument || document;
  }

  static inferLabelFromNearbyText(control, rootContext = null) {
    const sources = [];
    const MAX_CHARS = 80;
    ...
    const wrapper = control.closest && control.closest('[role="group"], [data-testid], [data-automation-id], .field, .form-field, .application-field, .input-field, .form-group, .input-wrapper');
    ...
    if (container) {
      ...
      container.querySelectorAll('label, [class*="label"], [class*="Label"], span, div, p, h1, h2, h3, h4').forEach(el => {
        ...
        if (/required$/i.test(txt) && txt.length < 10) return;
        candidates.push(txt);
      });

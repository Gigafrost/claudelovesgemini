...
class AIAgentSystem {
  ...
  async voteOnBestResponse(responses, prompt, context) {
    ...
    if (Object.keys(voteCounts).length > 0) {
      // Normal case: we have votes
      winner = Object.keys(voteCounts).reduce((a, b) =>
        voteCounts[a] > voteCounts[b] ? a : b
      );
    } else {
      // Fallback: No valid votes, pick first non-error response
      winner = this.agents.find(agent => responses[agent] && !responses[agent].error) || this.agents[0];
      console.warn('[AI Agent] No valid votes received, using fallback agent:', winner);
    }
